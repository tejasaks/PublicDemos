...
> **Instance and AG names:** You can rename the SQLServer resource (e.g. `sql-ag` → `my-sql`), the pod prefix, the AG name (`DisasterRecoveryAG`), and the database name (`CriticalDB`). If you do, update them consistently in:
> - `ag-deploy.yaml` — SQLServer `.metadata.name`, SQLServerAG `.metadata.name`, `.spec.availabilityGroup.name`, and the per-replica Service names
> - `ag-configure.sh` — the `PRIMARY`, `REPLICAS`, `AG_NAME`, `AG_RESOURCE_NAME`, and `DATABASE_NAME` variables at the top
