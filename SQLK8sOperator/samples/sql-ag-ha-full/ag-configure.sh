#!/usr/bin/env bash
# ============================================================================
# ag-configure.sh — Automated T-SQL Setup for Full HA Availability Group
# ============================================================================
# Shell script version of ag-configure.md. Executes all T-SQL steps required
# to create the ag1 Availability Group with two databases and a listener.
#
# USAGE:
...
NAMESPACE="mssql"
PRIMARY="sql-ag-full-0"
REPLICAS=("sql-ag-full-0" "sql-ag-full-1" "sql-ag-full-2")
SECONDARIES=("sql-ag-full-1" "sql-ag-full-2")
CONTAINER="mssql"

SA_PASSWORD='YourStrong@Passw0rd!'
AG_HEALTH_PASSWORD='H3althProbe!Pwd'
MASTER_KEY_PASSWORD='MasterKey@Passw0rd!'
REPLICA_LOGIN_PASSWORD='ReplicaLogin@Passw0rd!'

AG_NAME="ag1"
AG_RESOURCE_NAME="ag-full"
AG_LISTENER_NAME="ag-listener"
...
check_prerequisites() {
    ...
    if ! kubectl get secret ag-health-default -n "$NAMESPACE" &>/dev/null; then
        err "Secret ag-health-default not found. Apply ag-deploy.yaml first."
        exit 1
    fi
    ok "Secret ag-health-default exists"
}
...
step_1() {
    banner "Step 1: Create AG Health-Check Login (all replicas)"
    for pod in "${REPLICAS[@]}"; do
        info "Creating AG health-check login on $pod..."
        run_sql_check "$pod" "
CREATE LOGIN ag_health WITH PASSWORD = '$AG_HEALTH_PASSWORD';
GO
...
step_5() {
    banner "Step 5: Create Databases (primary: $PRIMARY)"
...
GO
BACKUP DATABASE [$db]
    TO DISK = '/var/opt/mssql/backup/${db}_init.bak'
...
step_6() {
    ...
    info "Creating $AG_NAME with databases: ${db_list}..."
    run_sql_check "$PRIMARY" "
...
"
    echo ""
    ok "Expected: All replicas CONNECTED + HEALTHY, both databases SYNCHRONIZED"
}
...
main() {
    echo -e "${BOLD}${CYAN}"
    echo "╔═══════════════════════════════════════════════════════════╗"
    echo "║  SQL Server AG — Full HA Configuration                   ║"
    echo "║  AG: $AG_NAME  |  Replicas: ${#REPLICAS[@]}  |  DBs: ${DATABASES[*]}"
    echo "╚═══════════════════════════════════════════════════════════╝"
    echo -e "${NC}"

    local cmd="${1:-all}"
    case "$cmd" in
        all)      check_prerequisites; step_1; step_2; step_3; step_4; step_5; step_6; step_7; step_8 ;;
        listener) check_prerequisites; step_9_listener ;;
        verify)   step_8 ;;
