#!/usr/bin/env bash
# ============================================================================
# ag-configure.sh — Automated T-SQL Setup for AG with Monitoring
# ============================================================================
# Identical T-SQL steps as the HA scenario (ProductionAG), plus monitoring
# verification commands for Prometheus and Grafana.
#
# USAGE:
...
#   ./ag-configure.sh monitoring   # Verify monitoring stack (step 10)
...
check_prerequisites() {
    ...
    if kubectl get namespace monitoring &>/dev/null; then
        ok "Monitoring namespace exists"
    else
        warn "Monitoring namespace not found — monitoring verification may fail"
    fi
}

# ============================================================================
# STEPS 1-8 (identical to HA scenario)
# ============================================================================
...
step_10_monitoring() {
    banner "Step 10: Verify Monitoring Stack"

    # Check Prometheus
    info "Checking Prometheus..."
    local prom_phase
    prom_phase=$(kubectl get pod -n monitoring -l app=prometheus -o jsonpath='{.items[0].status.phase}' 2>/dev/null) || true
    if [ "$prom_phase" = "Running" ]; then
        ok "Prometheus is Running"
    else
        warn "Prometheus: ${prom_phase:-not found}"
    fi

    # Check Grafana
    info "Checking Grafana..."
    local graf_phase
    graf_phase=$(kubectl get pod -n monitoring -l app=grafana -o jsonpath='{.items[0].status.phase}' 2>/dev/null) || true
    if [ "$graf_phase" = "Running" ]; then
        ok "Grafana is Running"
    else
        warn "Grafana: ${graf_phase:-not found}"
    fi

    # Check sql-exporter metrics
    info "Checking sql-exporter on primary..."
    local metrics
    metrics=$(kubectl exec "$PRIMARY" -n "$NAMESPACE" -c mssql -- \
        curl -s http://localhost:9399/metrics 2>/dev/null | head -5) || true
    if [ -n "$metrics" ]; then
        ok "sql-exporter responding on $PRIMARY"
    else
        warn "sql-exporter not responding on $PRIMARY (may need sql-exporter container)"
    fi

    # Check AG metrics
    info "Checking AG metrics..."
    local ag_metrics
    ag_metrics=$(kubectl exec "$PRIMARY" -n "$NAMESPACE" -c mssql -- \
        curl -s http://localhost:9399/metrics 2>/dev/null | grep "mssql_ag_" | head -3) || true
    if [ -n "$ag_metrics" ]; then
        ok "AG metrics available:"
        echo "$ag_metrics" | sed 's/^/     /'
    else
        warn "No AG metrics yet (AG may not be configured, or exporter not running)"
    fi
    ...
    # Check SQLServerAG status
    info "Checking SQLServerAG status..."
    kubectl get sqlserverag -n "$NAMESPACE" 2>/dev/null || warn "No SQLServerAG resources found"
}
...
usage() {
    ...
    echo "  monitoring  Verify monitoring stack (step 10)"
...
main() {
    echo -e "${BOLD}${CYAN}"
    echo "╔══════════════════════════════════════════════════════════════╗"
    echo "║  SQL Server AG with Monitoring — Configuration             ║"
    echo "╚══════════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
