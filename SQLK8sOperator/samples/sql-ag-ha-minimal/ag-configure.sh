#!/usr/bin/env bash
# ============================================================================
# ag-configure.sh — Automated T-SQL Setup for Minimal HA Availability Group
# ============================================================================
# Shell script version of ag-configure.md. Executes all T-SQL steps required
# to create the ag1 Availability Group.
#
# USAGE:
...
check_prerequisites() {
    ...
    if ! kubectl get secret ag-health-login -n "$NAMESPACE" &>/dev/null; then
        err "Secret ag-health-login not found. Apply ag-deploy.yaml first."
        exit 1
    fi
    ok "Secret ag-health-login exists"
}
...
main() {
    echo -e "${BOLD}${CYAN}"
    echo "╔═══════════════════════════════════════════════════════════╗"
    echo "║  SQL Server AG — Minimal HA Configuration                ║"
    echo "║  AG: $AG_NAME  |  Replicas: ${#REPLICAS[@]}  |  DB: $DATABASE_NAME"
    echo "╚═══════════════════════════════════════════════════════════╝"
    echo -e "${NC}"
